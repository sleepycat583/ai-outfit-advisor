...
融合 Qwen-Max 大模型、多模态视觉大模型 (Qwen-VL)、RAG 本地知识库 (Chroma)、DuckDuckGo 联网天气搜索，以及 LangGraph 驱动的问答编排，以“顶尖私人穿搭主理人”人设，提供图片识衣、数字衣橱管理、穿搭知识问答，以及基于天气的 7 天不重样周计划的完整 OOTD 解决方案。
...
LangGraph 问答编排：普通问答已迁移到 LangGraph 体系，支持 `stream_events()` 驱动前端状态展示，并保留兼容接口以降低改造风险。

显式 StateGraph（并存实现）：`rag.py` 中已新增显式 `StateGraph` 工作流，包含 `load_context / route_intent / fetch_weather / search_knowledge / generate` 五个节点，并通过 `use_graph=False/True` 开关与现有 ReAct Agent 路径并存。

Agent 联网决策：动态捕获用户所在城市，自主调用 DuckDuckGo 实时搜索近期天气，并对天气结果噪音、重复调用与超时场景做了收敛保护。
...
[用户鉴权 (app_main)] ──► [注册/登录] ──► [分配隔离沙箱 (user_id)]
       ...
       ┌────────────────────────────────────┴────────────────────────────────────┐
       ▼                                                                         ▼
[💬 穿搭问答端 (LangGraph + RAG)]                                      [👗 智能衣橱端]
       │                                                                         │
       ├─► 普通问答：stream_events() 驱动状态流                                   ├─► [上传照片] ─► [VLM 智能识衣]
       │       ├─► 默认路径：LangGraph create_react_agent                        │
       │       ├─► 可选路径：显式 StateGraph(use_graph=True)                     ├─► [手动录入表单]
       │       └─► 统一兼容：RagService.invoke()/stream_events()                 │
       │                                                                         ├─► [CSV 批量导入/导出备份]
       ├─► 📅 生成本周穿搭计划                                                    │
       │       ├─► 1. 联网查询属地未来天气                                        │
       │       ├─► 2. 检索 Chroma 获取穿搭法则                                    │
...
├── rag.py                      # RAG 核心：LangGraph 问答编排、显式 StateGraph、天气搜索工具、7天结构化规划
...
- 默认路径：`RagService(use_graph=False)`
  - 使用 LangGraph `create_react_agent` 路径
  - 通过 `stream_events()` 为 Streamlit UI 提供状态流

- 并存新路径：`RagService(use_graph=True)`
  - 使用显式 `StateGraph`
  - 节点顺序：
    - 天气场景：`load_context → route_intent → fetch_weather → search_knowledge → generate`
    - 非天气场景：`load_context → route_intent → search_knowledge → generate`
...
V2.4 架构演进与 LangGraph 增强 (Current)

LangGraph streaming：问答 UI 已从 callback 驱动逐步迁移为 `stream_events()` 事件流驱动，用户可见的“正在查天气 / 正在整理知识 / 正在生成建议”状态由 LangGraph 运行事件提供。

显式 StateGraph 并存：在保留现有 LangGraph ReAct Agent 路径的同时，引入了显式 `StateGraph` 工作流，并通过 `use_graph` 开关实现灰度切换与回滚友好设计。

天气工具收敛治理：增加天气搜索结果清洗、重复调用限制、递归保护，以及“非天气类穿搭问答不主动触发天气搜索”的边界控制。
